...
class ActionModule(ActionBase):
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        tool_path = self._shared_loader_obj.module_loader.find_plugin(
            'check_moodle')
        tool_path = os.path.join(os.path.dirname(tool_path if tool_path else ''),
                                 TOOL_FILENAME)
        if os.path.isfile(tool_path):
            module_args = dict(self._task.args)
            module_args.update({'moodle_tool_path': tool_path})
            ...
            raise AnsibleError('Failed to find the tool (%s) on path (%s) to run '
